...
class Subgraphs(Dataset):
    def __init__(self, root, mode, subgraph2label, n_way, k_shot, k_query, batchsz, args, adjs, h):
        ...
        if self.task_setup == 'Disjoint':
            ...
            if self.link_pred_mode:
                ...
                relative_idx_map_spt = dict(
                    zip(list(dictGraphs_spt.keys()), range(len(list(dictGraphs_spt.keys())))))
                ...
                relative_idx_map_qry = dict(
                    zip(list(dictGraphs_qry.keys()), range(len(list(dictGraphs_qry.keys())))))
                ...
                for i, (k, v) in enumerate(dictGraphsLabels.items()):
                    ...
                    for m, n in v.items():
                        ...
                        # [(graph 1)[(label1)[subgraph1, subgraph2, ...], (label2)[subgraph111, ...]], graph2: [[subgraph1, subgraph2, ...], [subgraph111, ...]] ]
    ...
    def create_batch_shared(self, batchsz):
        ...
        for b in range(batchsz):  # one loop generates one task
            ...
            selected_graph = np.random.choice(self.graph_num, 1, False)[
                0]  # select one graph
            ...
            for cls in selected_cls:
                ...
                try:
                    selected_subgraphs_idx = np.random.choice(
                        len(data[cls]), k_shot + k_query, False)
                    np.random.shuffle(selected_subgraphs_idx)
                    indexDtrain = np.array(
                        selected_subgraphs_idx[:k_shot])  # idx for Dtrain
                    indexDtest = np.array(
                        selected_subgraphs_idx[k_shot:])  # idx for Dtest
                    support_x.append(
                    ...
                    if len(data[cls]) >= k_shot:
                        selected_subgraphs_idx = np.array(
                            range(len(data[cls])))
                        np.random.shuffle(selected_subgraphs_idx)
                        indexDtrain = np.array(
                            selected_subgraphs_idx[:k_shot])  # idx for Dtrain
                        ...
                        while count <= num_more:
                            ...
                            query_tmp = query_tmp + \
                                [np.array(data[sub_cls])[idx]]
                            count += 1
                        query_x.append(query_tmp)
                    else:
                        print(
                            'each class in a graph must have larger than k_shot entities in the current model')
    ...
    def create_batch_LinkPred(self, batchsz):
        ...
        for b in range(batchsz):  # one loop generates one task
            ...
            selected_graph = np.random.choice(self.graph_num_spt, 1, False)[
                0]  # select one graph
            data_spt = self.data_label_spt[selected_graph]

            # for multiple graph setting, we select cls_num * k_shot nodes
            selected_cls_spt = np.array(list(range(len(data_spt))))
            np.random.shuffle(selected_cls_spt)

            data_qry = self.data_label_qry[selected_graph]

            # for multiple graph setting, we select cls_num * k_shot nodes
            selected_cls_qry = np.array(list(range(len(data_qry))))
            ...
            for cls in selected_cls_qry:
                ...
                selected_subgraphs_idx = np.random.choice(
                    len(data_qry[cls]), k_query, False)
                np.random.shuffle(selected_subgraphs_idx)
                query_x.append(np.array(data_qry[cls])[
                               selected_subgraphs_idx].tolist())
    ...
    def generate_subgraph(self, G, i, item, sublist, is_query=False):
        if item in self.subgraphs:
            ...
            if not is_query:
                h_hops_neighbor = np.union1d(h_hops_neighbor, shortest_paths)
    ...
    def generate_subgraph_link_pred(self, G, i, j, item, sublist, is_query=False):
        if item in self.subgraphs:
            return self.subgraphs[item]
        else:
            if not is_query:
            ...
            f_hop = [n.item() for n in G.in_edges(i)[0]]
            n_l = [[n.item() for n in G.in_edges(i)[0]] for i in f_hop]
            h_hops_neighbor1 = torch.tensor(
                list(set([item for sublist in n_l for item in sublist] + f_hop + [i]))).numpy()

            f_hop = [n.item() for n in G.in_edges(j)[0]]
            n_l = [[n.item() for n in G.in_edges(j)[0]] for i in f_hop]
            h_hops_neighbor2 = torch.tensor(
                list(set([item for sublist in n_l for item in sublist] + f_hop + [j]))).numpy()
    ...
    def __getitem__(self, index):
        ...
        if self.link_pred_mode:
            info = [self.generate_subgraph_link_pred(self.G[int(item.split('_')[0])], int(item.split('_')[1]), int(item.split('_')[2]), item, sublist)
                    for sublist in self.support_x_batch[index] for item in sublist]
        else:
            info = [self.generate_subgraph(self.G[int(item.split('_')[0])], int(item.split('_')[1]), item, sublist)
                    for sublist in self.support_x_batch[index] for item in sublist]
        ...
        if self.link_pred_mode:
            info = [self.generate_subgraph_link_pred(self.G[int(item.split('_')[0])], int(item.split('_')[1]), int(item.split('_')[2]), item, sublist, is_query=True)
                    for sublist in self.query_x_batch[index] for item in sublist]
        else:
            info = [self.generate_subgraph(self.G[int(item.split('_')[0])], int(item.split('_')[1]), item, sublist, is_query=True)
                    for sublist in self.query_x_batch[index] for item in sublist]
